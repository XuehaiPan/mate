...
.. code:: bash
    ...
    python3 -m examples.tarmac.target

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --target-agent examples.tarmac:TarMACTargetAgent \
        --target-kwargs '{ "checkpoint_path": "examples/tarmac/target/ray_results/TarMAC/latest-checkpoint" }'
"""
...
from examples.tarmac.target.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.tarmac.target.train'
            ),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    target_agent = TarMACTargetAgent(checkpoint_path=args.checkpoint_path)
