...
.. code:: bash
    ...
    python3 -m examples.tarmac.camera

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --camera-agent examples.tarmac:TarMACCameraAgent \
        --camera-kwargs '{ "checkpoint_path": "examples/tarmac/camera/ray_results/TarMAC/latest-checkpoint" }'
"""
...
from examples.tarmac.camera.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.tarmac.camera.train'
            ),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # Make agents ##############################################################
    camera_agent = TarMACCameraAgent(checkpoint_path=args.checkpoint_path)
