...
.. code:: bash
    ...
    python3 -m examples.iql.target

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --target-agent examples.iql:IQLTargetAgent \
        --target-kwargs '{ "checkpoint_path": "examples/iql/target/ray_results/IQL/latest-checkpoint" }'
"""
...
import mate
from examples.iql.target.agent import IQLTargetAgent
from examples.iql.target.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.iql.target.train'
            ),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    target_agent = IQLTargetAgent(checkpoint_path=args.checkpoint_path)
