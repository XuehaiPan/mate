...
.. code:: bash
    ...
    python3 -m examples.ippo.camera

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --camera-agent examples.ippo:IPPOCameraAgent \
        --camera-kwargs '{ "checkpoint_path": "examples/ippo/camera/ray_results/IPPO/latest-checkpoint" }'
"""
...
from examples.ippo.camera.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.ippo.camera.train'
            ),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # Make agents ##############################################################
    camera_agent = IPPOCameraAgent(checkpoint_path=args.checkpoint_path)
