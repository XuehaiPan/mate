...
from examples.tarmac.distributions import ActionDistributionWithMessage
from examples.tarmac.wrappers import ActionWithMessage
from examples.hrl.wrappers import HierarchicalCamera
from examples.utils import (RLlibMultiAgentAPI, RLlibMultiAgentCentralizedTraining, CustomMetricCallback,
...
def make_env(env_config):
    ...
    env = HierarchicalCamera(env, multi_selection=env_config.get('multi_selection', False),
                             frame_skip=env_config.get('frame_skip', 1))

    env = RLlibMultiAgentAPI(env)
    env = RLlibMultiAgentCentralizedTraining(env)
    env = ActionWithMessage(env, message_dim=env_config['message_dim'])
    return env


tune.register_env('mate-hrl.tarmac.camera', make_env)
...
config = {
    ...
    'model': {
        'max_seq_len': 25,
        'custom_action_dist': 'action_distribution_with_message',
        'custom_model': TarMACModel,
        'custom_model_config': {
            **MODEL_DEFAULTS,
