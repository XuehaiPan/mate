...
r"""
Example of hierarchical TarMAC agents for the Multi-Agent Tracking Environment.

.. code:: bash

    python3 -m examples.hrl.tarmac.camera

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --camera-agent examples.hrl:HRLTarMACCameraAgent \
        --camera-kwargs '{ "checkpoint_path": "examples/hrl/tarmac/camera/ray_results/HRL-TarMAC/latest-checkpoint" }'
"""
...
from examples.hrl.tarmac.camera.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.hrl.tarmac.camera.train'
            ),
            file=sys.stderr
        )
        sys.exit(1)
    ...
    # Make agents ##############################################################
    camera_agent = HRLTarMACCameraAgent(checkpoint_path=args.checkpoint_path)
