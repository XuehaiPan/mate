...
r"""
Example of hierarchical IPPO agents for the Multi-Agent Tracking Environment.

.. code:: bash

    python3 -m examples.hrl.ippo.camera

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --camera-agent examples.hrl:HRLIPPOCameraAgent \
        --camera-kwargs '{ "checkpoint_path": "examples/hrl/ippo/camera/ray_results/HRL-IPPO/latest-checkpoint" }'
"""
...
from examples.hrl.ippo.camera.agent import HRLIPPOCameraAgent
from examples.hrl.ippo.camera.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.hrl.ippo.camera.train'
            ),
            file=sys.stderr
        )
        sys.exit(1)
    ...
    # Make agents ##############################################################
    camera_agent = HRLIPPOCameraAgent(checkpoint_path=args.checkpoint_path)
    ...
    for i in range(MAX_EPISODE_STEPS):
        ...
        selections = [(agent.index, agent.last_selection, agent.last_mask)
        ...
        render_callback = functools.partial(HierarchicalCamera.render_selection_callback, selections=selections)
