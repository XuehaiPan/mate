...
r"""
Example of hierarchical QMIX agents for the Multi-Agent Tracking Environment.

.. code:: bash

    python3 -m examples.hrl.qmix.camera

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --camera-agent examples.hrl:HRLQMIXCameraAgent \
        --camera-kwargs '{ "checkpoint_path": "examples/hrl/qmix/camera/ray_results/HRL-QMIX/latest-checkpoint" }'
"""
...
from examples.hrl.qmix.camera.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.hrl.qmix.camera.train'
            ),
            file=sys.stderr
        )
        sys.exit(1)
    ...
    # Make agents ##############################################################
    camera_agent = HRLQMIXCameraAgent(checkpoint_path=args.checkpoint_path)
