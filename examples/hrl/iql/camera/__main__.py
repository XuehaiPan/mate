...
r"""
Example of hierarchical IQL agents for the Multi-Agent Tracking Environment.

.. code:: bash

    python3 -m examples.hrl.iql.camera

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --camera-agent examples.hrl:HRLIQLCameraAgent \
        --camera-kwargs '{ "checkpoint_path": "examples/hrl/iql/camera/ray_results/HRL-IQL/latest-checkpoint" }'
"""
...
from examples.hrl.iql.camera.agent import HRLIQLCameraAgent
from examples.hrl.iql.camera.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.hrl.iql.camera.train'
            ),
            file=sys.stderr
        )
        sys.exit(1)
    ...
    # Make agents ##############################################################
    camera_agent = HRLIQLCameraAgent(checkpoint_path=args.checkpoint_path)
