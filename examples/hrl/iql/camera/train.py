...
# Run: python3 -m examples.hrl.iql.camera.train
...
from examples.hrl.iql.camera.config import config
...
def train(
    ...
    seed=None, timesteps_total=None, buffer_capacity=5E5
):
    ...
    if WandbLoggerCallback.is_available():
        ...
        group = group or f'hrl.iql.camera.{experiment.name}'
    ...
    experiment.spec['config'].setdefault('replay_buffer_config', {})
    experiment.spec['config']['replay_buffer_config'].update(capacity=round(buffer_capacity))
...
def main():
    ...
    parser.add_argument('--buffer-capacity', type=float, metavar='STEP', default=5E5,
                        help='capacity for the replay buffer (default: %(default).1E environment steps)')
