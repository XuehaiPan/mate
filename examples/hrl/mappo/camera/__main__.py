...
r"""
Example of hierarchical MAPPO agents for the Multi-Agent Tracking Environment.

.. code:: bash

    python3 -m examples.hrl.mappo.camera

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --camera-agent examples.hrl:HRLMAPPOCameraAgent \
        --camera-kwargs '{ "checkpoint_path": "examples/hrl/mappo/camera/ray_results/HRL-MAPPO/latest-checkpoint" }'
"""
...
from examples.hrl.mappo.camera.train import experiment
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.hrl.mappo.camera.train'
            ),
            file=sys.stderr
        )
        sys.exit(1)
