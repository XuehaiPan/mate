...
from examples.i2c.target.config import config as _config, make_env as _make_env
from examples.utils import RLlibPolicyMixIn


class I2CTargetAgent(RLlibPolicyMixIn, mate.TargetAgentBase):
    """I2C Target Agent
    ...
    def send_requests(self):
        ...
        for i in np.flatnonzero(comm_mask):
            message = self.pack_message(content=None,
                                        recipient=(self.index + 1 + i) % self.num_teammates)
    ...
    def send_responses(self):
        ...
        for request in self.last_requests:
            message = self.pack_message(content=self.last_observation.copy(),
    ...
    def preprocess_observation(self, observation):
        ...
        joint_observation = np.zeros(shape=(self.num_teammates, self.observation_dim),
        ...
        cycled_preprocessed_joint_observation = preprocessed_joint_observation + preprocessed_joint_observation
        dummy_preprocessed_observation[self.flat_obs_slices['others_joint_observation']] \
            = np.ravel(cycled_preprocessed_joint_observation[self.index + 1:self.index + self.num_teammates])
