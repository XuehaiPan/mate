...
.. code:: bash
    ...
    python3 -m examples.maddpg.target

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --target-agent examples.maddpg:MADDPGTargetAgent \
        --target-kwargs '{ "checkpoint_path": "examples/maddpg/target/ray_results/MADDPG/latest-checkpoint" }'
"""
...
from examples.maddpg.target.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.maddpg.target.train'
            ),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    target_agent = MADDPGTargetAgent(checkpoint_path=args.checkpoint_path)
