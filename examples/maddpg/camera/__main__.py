...
.. code:: bash
    ...
    python3 -m examples.maddpg.camera

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --camera-agent examples.maddpg:MADDPGCameraAgent \
        --camera-kwargs '{ "checkpoint_path": "examples/maddpg/camera/ray_results/MADDPG/latest-checkpoint" }'
"""
...
from examples.maddpg.camera.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.maddpg.camera.train'
            ),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # Make agents ##############################################################
    camera_agent = MADDPGCameraAgent(checkpoint_path=args.checkpoint_path)
