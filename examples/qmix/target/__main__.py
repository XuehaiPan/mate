...
r"""
Example of QMIX agents for the Multi-Agent Tracking Environment.

.. code:: bash

    python3 -m examples.qmix.target

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --target-agent examples.qmix:QMIXTargetAgent \
        --target-kwargs '{ "checkpoint_path": "examples/qmix/target/ray_results/QMIX/latest-checkpoint" }'
"""
...
from examples.qmix.target.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.qmix.target.train'
            ),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    target_agent = QMIXTargetAgent(checkpoint_path=args.checkpoint_path)
