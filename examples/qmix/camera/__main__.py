...
.. code:: bash
    ...
    python3 -m examples.qmix.camera

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --camera-agent examples.qmix:QMIXCameraAgent \
        --camera-kwargs '{ "checkpoint_path": "examples/qmix/camera/ray_results/QMIX/latest-checkpoint" }'
"""
...
from examples.qmix.camera.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.qmix.camera.train'
            ),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # Make agents ##############################################################
    camera_agent = QMIXCameraAgent(checkpoint_path=args.checkpoint_path)
