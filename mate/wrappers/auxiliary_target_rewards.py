...
from mate.wrappers.single_team import SingleTeamHelper, MultiCamera
...
class AuxiliaryTargetRewards(gym.Wrapper, metaclass=WrapperMeta):
    ...
    def __init__(self, env: MultiAgentEnvironmentType,
        ...
        assert not isinstance(env, AuxiliaryTargetRewards), (
    ...
    def step(  # pylint: disable=too-many-locals,too-many-branches
        ...
        if 'soft_coverage_score' in self.coefficients:
            self.soft_coverage_score_matrix = AuxiliaryCameraRewards.compute_soft_coverage_scores(self.unwrapped)
        ...
        for t, (raw_reward, info) in enumerate(zip(tuple(target_rewards), target_infos)):
            ...
            for key, coefficient in self.coefficients.items():
                if callable(coefficient):
                    coefficient = coefficient(t, self.episode_id, self.episode_step,
