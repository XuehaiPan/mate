...
class RelativeCoordinates(gym.ObservationWrapper, metaclass=WrapperMeta):
    ...
    def observation(
        ...
        camera_joint_observation = self.convert_coordinates(camera_joint_observation, team=Team.CAMERA)
        target_joint_observation = self.convert_coordinates(target_joint_observation, team=Team.TARGET)
        return camera_joint_observation, target_joint_observation

    def convert_coordinates(self, observation: np.ndarray, team: Team) -> np.ndarray:  # pylint: disable=missing-function-docstring
        return convert_coordinates(observation, team=team,
