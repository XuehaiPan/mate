...
from mate.wrappers.typing import WrapperMeta, BaseEnvironmentType, assert_base_environment
...
class DiscreteCamera(gym.ActionWrapper, metaclass=WrapperMeta):
    ...
    def __init__(self, env: BaseEnvironmentType, levels: int = 5) -> None:
        ...
        assert not isinstance(env, DiscreteCamera), (
        ...
        self.camera_joint_action_space = spaces.Tuple(spaces=(self.camera_action_space,) * env.num_cameras)
        self.action_space = spaces.Tuple(spaces=(self.camera_joint_action_space,
                                                 env.target_joint_action_space))

        self.action_high = np.asarray([env.camera_rotation_step, env.camera_zooming_step], dtype=np.float64)
    ...
    def action(self, action: Tuple[np.ndarray, np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        camera_joint_action_discrete = np.asarray(camera_joint_action_discrete, dtype=np.int64).ravel()
        ...
        camera_joint_action_continuous = self.action_high * self.normalized_action_grid[camera_joint_action_discrete]
        return camera_joint_action_continuous, target_joint_action

    def reverse_action(self, action: Tuple[np.ndarray, np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
        """Convert joint action of cameras from continuous to discrete."""

        camera_joint_action_continuous, target_joint_action = action
        camera_joint_action_continuous = np.asarray(camera_joint_action_continuous, dtype=np.float64)
        camera_joint_action_continuous = camera_joint_action_continuous.reshape(self.num_cameras,
                                                                                consts.CAMERA_ACTION_DIM)

        camera_joint_action_discrete = indices_of_nearest_grid_point(camera_joint_action_continuous / self.action_high,
                                                                     self.normalized_action_grid)
    ...
    def discrete_action_grid(levels):  # pylint: disable=missing-function-docstring
        ...
        normalized_action_grid = np.stack(
            np.meshgrid(np.linspace(start=-1.0, stop=+1.0, num=levels, endpoint=True),
...
class DiscreteTarget(gym.ActionWrapper, metaclass=WrapperMeta):
    ...
    def __init__(self, env: BaseEnvironmentType, levels: int = 5) -> None:
        assert_base_environment(env)
        assert not isinstance(env, DiscreteTarget), (
            f'You should not use wrapper `{self.__class__}` more than once. '
            f'Got env = {env}.'
        )
        assert levels >= 3 and levels % 2 == 1, (
        ...
        self.target_joint_action_space = spaces.Tuple(spaces=(self.target_action_space,) * env.num_targets)
        self.action_space = spaces.Tuple(spaces=(env.camera_joint_action_space,
                                                 self.target_joint_action_space))

        self.action_high = env.target_step_size * np.ones((env.num_targets, consts.TARGET_ACTION_DIM), dtype=np.float64)
    ...
    def action(self, action: Tuple[np.ndarray, np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        target_joint_action_discrete = np.asarray(target_joint_action_discrete, dtype=np.int64).ravel()
        ...
        target_joint_action_continuous = self.action_high * self.normalized_action_grid[target_joint_action_discrete]
    ...
    def reverse_action(self, action: Tuple[np.ndarray, np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        target_joint_action_continuous = np.asarray(target_joint_action_continuous, dtype=np.float64)
        target_joint_action_continuous = target_joint_action_continuous.shape(self.num_targets,
                                                                              consts.TARGET_ACTION_DIM)

        target_joint_action_discrete = indices_of_nearest_grid_point(target_joint_action_continuous / self.action_high,
                                                                     self.normalized_action_grid)
    ...
    def discrete_action_grid(levels):  # pylint: disable=missing-function-docstring
        ...
        action_grid = np.stack(np.meshgrid(np.linspace(start=-1.0, stop=+1.0, num=levels, endpoint=True),
                                           np.linspace(start=-1.0, stop=+1.0, num=levels, endpoint=True)),
                               axis=-1).reshape(-1, consts.TARGET_ACTION_DIM)
