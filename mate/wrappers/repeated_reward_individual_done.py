...
from mate.wrappers.typing import WrapperMeta, MultiAgentEnvironmentType, assert_multi_agent_environment  # pylint: disable=cyclic-import
...
class RepeatedRewardIndividualDone(gym.Wrapper, metaclass=WrapperMeta):
    ...
    def __init__(self, env: MultiAgentEnvironmentType, target_done_at_destination=False) -> None:
        ...
        assert not isinstance(env, RepeatedRewardIndividualDone),(
        ...
        from mate.wrappers.single_team import SingleTeamHelper  # pylint: disable=import-outside-toplevel,cyclic-import

        self.single_team = isinstance(env, SingleTeamHelper)

    def step(self, action: Union[Tuple[np.ndarray, np.ndarray],
                                 np.ndarray]) -> Union[Tuple[Tuple[np.ndarray, np.ndarray],
        ...
        if self.single_team:
            ...
            reward = ([camera_team_reward] * self.num_cameras, [target_team_reward] * self.num_targets)
