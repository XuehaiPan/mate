...
class MoreTrainingInformation(gym.Wrapper, metaclass=WrapperMeta):
    ...
    def __init__(self, env: BaseEnvironmentType) -> None:
        ...
        assert not isinstance(env, MoreTrainingInformation), (
            f'You should not use wrapper `{self.__class__}` more than once.'
        )

        super().__init__(env)

    def step(  # pylint: disable=too-many-locals
        self,
        ...
        camera_states_private = camera_joint_observation[..., offset:offset + consts.CAMERA_STATE_DIM_PRIVATE]
        target_states_private = target_joint_observation[..., offset:offset + consts.TARGET_STATE_DIM_PRIVATE]
        ...
        for c, camera_info in enumerate(camera_infos):
            camera_info.update(num_tracked=self.camera_target_view_mask[c, ...].sum(),
        ...
        for t, target_info in enumerate(target_infos):
            ...
            warehouse_distances = np.maximum(self.target_warehouse_distances[t] - consts.WAREHOUSE_RADIUS, 0.0,
                                             dtype=np.float64)
            goal_distance = (warehouse_distances[goal] if goal >= 0 else consts.TERRAIN_WIDTH / 2.0)
            target_info.update(goal=goal,
