...
class RescaledObservation(gym.ObservationWrapper, metaclass=WrapperMeta):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, env: MateEnvironmentType) -> None:
        ...
        from mate.wrappers.single_team import SingleTeamHelper, SingleTeamSingleAgent  # pylint: disable=import-outside-toplevel
        ...
        camera_joint_observation_space = spaces.Tuple(spaces=(camera_observation_space,) * env.num_cameras)
        target_joint_observation_space = spaces.Tuple(spaces=(target_observation_space,) * env.num_targets)

        if self.single_team:
            self.teammate_observation_space, self.opponent_observation_space \
                = env.swap(camera_observation_space, target_observation_space)
            self.teammate_joint_observation_space, self.opponent_joint_observation_space \
                = env.swap(camera_joint_observation_space, target_joint_observation_space)
            ...
            if isinstance(env, SingleTeamSingleAgent):
                ...
                self.observation_space = spaces.Tuple(spaces=(self.teammate_observation_space,) * env.num_teammates)
        else:
            self.camera_observation_space = camera_observation_space
            self.target_observation_space = target_observation_space
            self.camera_joint_observation_space = camera_joint_observation_space
    ...
    def observation(
        self,
        observation: Union[Tuple[np.ndarray, np.ndarray], np.ndarray]
        ...
        camera_joint_observation = self.rescale_observation(camera_joint_observation, team=Team.CAMERA)
        target_joint_observation = self.rescale_observation(target_joint_observation, team=Team.TARGET)
        return camera_joint_observation, target_joint_observation

    def rescale_observation(self, observation: np.ndarray, team: Team) -> np.ndarray:  # pylint: disable=missing-function-docstring
        return rescale_observation(observation, team=team,
