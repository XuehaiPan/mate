...
class SharedFieldOfView(gym.ObservationWrapper, metaclass=WrapperMeta):
    ...
    def __init__(self, env: BaseEnvironmentType, team: Literal['both', 'camera', 'target', 'none'] = 'both') -> None:
        assert_base_environment(env)
        assert team in ('both', 'camera', 'target', 'none'), (
            f'Invalid argument team {team:!r}. Expect one of {("both", "camera", "target", "none")}.'
        )
        from mate.wrappers.relative_coordinates import RelativeCoordinates  # pylint: disable=import-outside-toplevel
        ...
        from mate.wrappers.rescaled_observation import RescaledObservation  # pylint: disable=import-outside-toplevel
        ...
        self.target_empty_bits_slice = slice(self.target_indices[2] - consts.NUM_WAREHOUSES, self.target_indices[2])
    ...
    def observation(self, observation: Tuple[np.ndarray, np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        camera_states_public = camera_joint_observation[..., offset: offset + consts.CAMERA_STATE_DIM_PUBLIC]
        target_states_public = target_joint_observation[..., offset: offset + consts.TARGET_STATE_DIM_PUBLIC]
        camera_states_public_flagged = np.hstack([camera_states_public, np.ones((self.num_cameras, 1))])
        target_states_public_flagged = np.hstack([target_states_public, np.ones((self.num_targets, 1))])
        ...
        if self.shared_camera:
            ...
            camera_joint_observation[..., self.camera_slices['opponent_states_with_mask']] \
                = np.where(shared_target_mask, target_states_public_flagged, 0.0).ravel()[np.newaxis, ...]
            camera_joint_observation[..., self.camera_slices['obstacle_states_with_mask']] \
                = np.where(shared_obstacle_mask, obstacle_states_flagged, 0.0).ravel()[np.newaxis, ...]
            camera_joint_observation[..., self.camera_slices['teammate_states_with_mask']] \
        ...
        if self.shared_target:
            ...
            target_joint_observation[..., self.target_empty_bits_slice] \
                = shared_empty_bits[np.newaxis, ...]
            target_joint_observation[..., self.target_slices['opponent_states_with_mask']] \
                = np.where(shared_camera_mask, camera_states_public_flagged, 0.0).ravel()[np.newaxis, ...]
            target_joint_observation[..., self.target_slices['obstacle_states_with_mask']] \
                = np.where(shared_obstacle_mask, obstacle_states_flagged, 0.0).ravel()[np.newaxis, ...]
            target_joint_observation[..., self.target_slices['teammate_states_with_mask']] \
