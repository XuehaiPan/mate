...
def generate(path, num_cameras, num_targets, num_obstacles,
             num_cargoes_per_target=8, obstacle_transmittance=0.1,
             seed=0, plot=False):
    ...
    assert file_ext in ('.json', '.yaml', '.yml'), (
        f'Unsupported file extension {file_ext}.'
    )

    print('Generating configuration with {} camera{}, {} target{} and {} obstacle{} to "{}".'.format(
        num_cameras, ('s' if num_cameras > 1 else ''),
        num_targets, ('s' if num_targets > 1 else ''),
        num_obstacles, ('s' if num_obstacles > 1 else ''),
        path
    ))
        ...
        try:
            with tqdm.trange(MAX_ITERATIONS) as pbar:
                for i in pbar:
                    ...
                    distance_horizontal = torch.minimum((1 - locations[..., 0]).abs(), (1 + locations[..., 0]).abs())
                    distance_vertical = torch.minimum((1 - locations[..., 1]).abs(), (1 + locations[..., 1]).abs())
                    ...
                    loss = max_distance
                    regularizer = - (0.001 * distance_to_other.min() + 0.1 * distance_to_border.min())
                    loss += regularizer
                    ...
                    if plot:
                        ...
                        color = (1 + indices_to_nearest.detach().cpu().numpy().astype(np.float64)) / (num_cameras + 1)
                        contour = ax1.tricontourf(*MESH_NUMPY.T, color, levels=num_cameras + 1,
                                                  vmin=0.0, vmax=1.0, cmap='hsv', zorder=1)
    ...
    config['target']['location_random_range'] = [[-0.5 * SCALE, +0.5 * SCALE, -0.5 * SCALE, +0.5 * SCALE]] * num_targets
    config['target']['sight_range'] = config['camera']['max_sight_range'] / 2.0

    if num_obstacles > 0:
        config['obstacle']['location_random_range'] = [[-SCALE, +SCALE, -SCALE, +SCALE]] * num_obstacles
...
def main():
    parser = argparse.ArgumentParser(prog='python -m mate.assets.generator',
    ...
    parser.add_argument('--path', type=str, metavar='PATH', default='config.yaml',
                        help='Path to save configuration file. (default: %(default)s)')
    parser.add_argument('--num-cameras', type=int, metavar='CAMERA', default=4,
                        help='Number of the cameras in the environment. (default: %(default)d)')
    parser.add_argument('--num-targets', type=int, metavar='TARGET', default=8,
                        help='Number of the targets in the environment. (default: %(default)d)')
    parser.add_argument('--num-obstacles', type=int, metavar='OBSTACLE', default=0,
                        help='Number of the obstacles in the environment. (default: %(default)d)')
    parser.add_argument('--num-cargoes-per-target', type=int, metavar='CARGO', default=8,
                        help='Average number of cargoes (>=4) per target. (default: %(default)d)')
    parser.add_argument('--obstacle-transmittance', type=float, metavar='FACTOR', default=0.1,
    ...
    parser.add_argument('--plot', action='store_true',
                        help='Show iteration result plots.')
